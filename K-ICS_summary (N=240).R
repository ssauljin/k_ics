...
prd240_ggh  <- cbind(prd240.vstd, prd240.vemp, prd240.varc, prd240.velp, pop.var_true) 

load("K-ICS_analysis (J=500, true=g cop, g dist, mid cor, N=240).RData")
prd240_ggm  <- cbind(prd240.vstd, prd240.vemp, prd240.varc, prd240.velp, pop.var_true) 

load("K-ICS_analysis (J=500, true=g cop, g dist, low cor, N=240).RData")
prd240_ggl  <- cbind(prd240.vstd, prd240.vemp, prd240.varc, prd240.velp, pop.var_true) 

load("K-ICS_analysis (J=500, true=Gumbel cop, 4 dist, high cor, N=240).RData")
prd240_a4h  <- cbind(prd240.vstd, prd240.vemp, prd240.varc, prd240.velp, pop.var_true) 

load("K-ICS_analysis (J=500, true=Gumbel cop, 4 dist, mid cor, N=240).RData")
prd240_a4m  <- cbind(prd240.vstd, prd240.vemp, prd240.varc, prd240.velp, pop.var_true) 

load("K-ICS_analysis (J=500, true=Gumbel cop, 4 dist, low cor, N=240).RData")
prd240_a4l  <- cbind(prd240.vstd, prd240.vemp, prd240.varc, prd240.velp, pop.var_true) 

load("K-ICS_analysis (J=500, true=t cop, 4 dist, high cor, N=240).RData")
prd240_t4h  <- cbind(prd240.vstd, prd240.vemp, prd240.varc, prd240.velp, pop.var_true) 

load("K-ICS_analysis (J=500, true=t cop, 4 dist, mid cor, N=240).RData")
prd240_t4m  <- cbind(prd240.vstd, prd240.vemp, prd240.varc, prd240.velp, pop.var_true) 

load("K-ICS_analysis (J=500, true=t cop, 4 dist, low cor, N=240).RData")
prd240_t4l  <- cbind(prd240.vstd, prd240.vemp, prd240.varc, prd240.velp, pop.var_true) 
...
prt240_ggh     <- as.data.frame(cbind(as.vector(prd240_ggh[,1:4]-prd240_ggh[,5]),
                                     rep(1, 500)))
prt240_ggh[,2] <- labels
...
prt240_ggm     <- as.data.frame(cbind(as.vector(prd240_ggm[,1:4]-prd240_ggm[,5]),
                                     rep(1, 500)))
prt240_ggm[,2] <- labels
...
prt240_ggl     <- as.data.frame(cbind(as.vector(prd240_ggl[,1:4]-prd240_ggl[,5]),
                                     rep(1, 500)))
prt240_ggl[,2] <- labels
...
prt240_a4h     <- as.data.frame(cbind(as.vector(prd240_a4h[,1:4]-prd240_a4h[,5]),
                                     rep(1, 500)))
prt240_a4h[,2] <- labels
...
prt240_a4m     <- as.data.frame(cbind(as.vector(prd240_a4m[,1:4]-prd240_a4m[,5]),
                                     rep(1, 500)))
prt240_a4m[,2] <- labels
...
prt240_a4l     <- as.data.frame(cbind(as.vector(prd240_a4l[,1:4]-prd240_a4l[,5]),
                                     rep(1, 500)))
prt240_a4l[,2] <- labels
...
prt240_t4h     <- as.data.frame(cbind(as.vector(prd240_t4h[,1:4]-prd240_t4h[,5]),
                                     rep(1, 500)))
prt240_t4h[,2] <- labels
...
prt240_t4m     <- as.data.frame(cbind(as.vector(prd240_t4m[,1:4]-prd240_t4m[,5]),
                                     rep(1, 500)))
prt240_t4m[,2] <- labels
...
prt240_t4l     <- as.data.frame(cbind(as.vector(prd240_t4l[,1:4]-prd240_t4l[,5]),
                                     rep(1, 500)))
prt240_t4l[,2] <- labels
